...
object ShapeFileReader {
  ...
  def readSimpleFeaturesHDFS(path: String) = {
    // Extract the features as GeoTools 'SimpleFeatures'
    //val url = s"file://${new File(path).getAbsolutePath}"
    val url = s"${path}"
    println("Reading HDFS URL: "+url)
    val ds = new ShapefileDataStore(new URL(url))
    val ftItr: SimpleFeatureIterator = ds.getFeatureSource.getFeatures.features
  ...
  def readMultiPolygonFeaturesHDFS(path: String): Seq[MultiPolygonFeature[Map[String,Object]]] =
    //readSimpleFeaturesHDFS(path)
    readSimpleFeatures(new URL(path))
